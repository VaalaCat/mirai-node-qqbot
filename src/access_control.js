...
async function init_database() {
	return new Promise(async (resolve, reject) => {
		...
		let database = new sqlite3.Database("./data/permission.db", (error) => { if (error) { console.log("数据库启动错误"); } });
		database.run("create table if not exists users(id text primary key not null, base int not null, allow text , ban text )");
		database.run("create table if not exists groups(id text primary key not null, base int not null, allow text , ban text )");
